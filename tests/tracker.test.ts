...
describe('tracker', () => {
  ...
  describe('resolvePath', () => {
    ...
    it('should handle file:// URIs', () => {
      expect(resolvePath(base, 'file:///home/dev/project/src/main.ts')).toBe(
        '/home/dev/project/src/main.ts',
      );
    });
  ...
  describe('isIgnoredPath', () => {
    ...
    it('should ignore internal gemini MCP tool schemas and instructions', () => {
      expect(isIgnoredPath('/home/dev/.gemini/antigravity/mcp/coolify/deploy.json')).toBe(true);
      expect(isIgnoredPath('/home/dev/.gemini/antigravity/mcp/browseros/tabs.json')).toBe(true);
    });

    it('should ignore internal skills and config directories', () => {
      expect(isIgnoredPath('/home/dev/.gemini/config/skills/find-skills/SKILL.md')).toBe(true);
      expect(isIgnoredPath('/home/dev/.gemini/config/config.json')).toBe(true);
    });

    it('should ignore node_modules, .git, and temp paths', () => {
      expect(isIgnoredPath('/home/dev/my-project/node_modules/lodash/index.js')).toBe(true);
    ...
    it('should NOT ignore legitimate project source files', () => {
      expect(isIgnoredPath('/home/dev/.projects/chronova-antigravity-plugin/src/types.ts')).toBe(
        false,
      );
      expect(isIgnoredPath('/home/dev/projects/my-app/src/components/Button.tsx')).toBe(false);
    });
  });

  describe('findMatchingWorkspace', () => {
    const workspaces = ['/home/dev/.projects/project-a', '/home/dev/.projects/project-b'];

    it('should match file to its workspace', () => {
      expect(findMatchingWorkspace('/home/dev/.projects/project-a/src/index.ts', workspaces)).toBe(
        '/home/dev/.projects/project-a',
      );
      expect(findMatchingWorkspace('/home/dev/.projects/project-b/README.md', workspaces)).toBe(
      ...
      const result = parseToolCall(
        {
          name: 'view_file',
          args: {
            AbsolutePath: '/home/dev/.gemini/antigravity/mcp/coolify/deploy.json',
          },
        },
        base,
        workspacePaths,
